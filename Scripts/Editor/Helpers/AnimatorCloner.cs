...
namespace Shadster.AvatarTools.VRLabs.AV3Manager //COPIED FROM VRLABS AV3MANAGER 2.0.24, modified for more controlled use
{
    public static class AnimatorCloner
    {
        ...
        public static void CopyControllerLayer(AnimatorController source, int sourceLayerIndex, AnimatorController dest)
        {
            ...
            if (sourceLayerIndex < 0 || sourceLayerIndex >= sourceLayers.Length)
            {
                Debug.LogError("Invalid source layer index.");
                return;
            }

            AnimatorControllerLayer newL = CloneLayer(source.layers[sourceLayerIndex], sourceLayerIndex == 0);
            //newL.name = dest.MakeUniqueLayerName(newL.name);// MakeLayerNameUnique(newL.name, mainController);
        ...
        public static void DeleteExistingControllerLayer(AnimatorController controller, string layerName)
        {
